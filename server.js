...
db.serialize(() => {
  ...
  db.run(
    "ALTER TABLE assignments ADD COLUMN schedule_days TEXT NOT NULL DEFAULT 'Monday-Sunday (Daily)'",
    (err) => {
      if (err && !String(err.message || '').includes('duplicate column name')) {
        // eslint-disable-next-line no-console
        console.error(err);
  ...
  db.run('ALTER TABLE assignments ADD COLUMN schedule_start_date TEXT', (err) => {
    if (err && !String(err.message || '').includes('duplicate column name')) {
      // eslint-disable-next-line no-console
      console.error(err);
    }
  });
  db.run('ALTER TABLE assignments ADD COLUMN schedule_end_date TEXT', (err) => {
    if (err && !String(err.message || '').includes('duplicate column name')) {
      // eslint-disable-next-line no-console
      console.error(err);
    }
  });
  db.run('ALTER TABLE assignments ADD COLUMN schedule_dates_json TEXT', (err) => {
    if (err && !String(err.message || '').includes('duplicate column name')) {
      // eslint-disable-next-line no-console
      console.error(err);
    }
  });
  db.run("ALTER TABLE assignments ADD COLUMN practice_mode TEXT NOT NULL DEFAULT 'minutes'", (err) => {
    if (err && !String(err.message || '').includes('duplicate column name')) {
      // eslint-disable-next-line no-console
      console.error(err);
    }
  });
  db.run("ALTER TABLE assignments ADD COLUMN practice_target INTEGER NOT NULL DEFAULT 10", (err) => {
    if (err && !String(err.message || '').includes('duplicate column name')) {
      // eslint-disable-next-line no-console
      console.error(err);
    }
  });
  db.run('ALTER TABLE assignments ADD COLUMN is_archived INTEGER NOT NULL DEFAULT 0', (err) => {
    if (err && !String(err.message || '').includes('duplicate column name')) {
      // eslint-disable-next-line no-console
      console.error(err);
    }
  });
  db.run('ALTER TABLE assignments ADD COLUMN archived_at DATETIME', (err) => {
    if (err && !String(err.message || '').includes('duplicate column name')) {
      // eslint-disable-next-line no-console
      console.error(err);
    }
...
function parseScheduleDatesFromJson(rawJson) {
  ...
  try {
    ...
    return normalizeScheduleDates(parsed);
  } catch (_error) {
    return null;
  }
}
...
async function ensureDateChecksForAssignment(assignment) {
  const scheduleDates = getScheduleDatesForAssignment(assignment);
  for (const dateText of scheduleDates) {
    await runQuery(
      `
      INSERT OR IGNORE INTO assignment_date_checks (assignment_id, task_date)
      VALUES (?, ?)
      `,
      [assignment.id, dateText]
    );
  }
}
...
app.get('/api/users', async (_req, res) => {
  try {
    const users = await allQuery('SELECT id, name, role, created_at FROM users ORDER BY role, name');
    res.json(users);
  } catch (error) {
...
app.post('/api/users', async (req, res) => {
  ...
  try {
    const existing = await getQuery('SELECT * FROM users WHERE name = ? AND role = ?', [name.trim(), role]);
    if (existing) {
      if (!existing.password) {
        const hash = await bcrypt.hash(String(password), 10);
        await runQuery('UPDATE users SET password = ? WHERE id = ?', [hash, existing.id]);
        const updatedUser = await getQuery('SELECT id, name, role, created_at FROM users WHERE id = ?', [existing.id]);
        const token = generateToken();
        authTokens.set(token, updatedUser.id);
        return res.json({ ...updatedUser, token });
      }
      // Support migrating legacy plaintext passwords to bcrypt on first successful login
    ...
    const result = await runQuery('INSERT INTO users (name, role, password) VALUES (?, ?, ?)', [
      name.trim(),
      role,
      hashedPassword
    ]);
    const user = await getQuery('SELECT id, name, role, created_at FROM users WHERE id = ?', [result.id]);
    const token = generateToken();
...
app.get('/api/assignments', requireAuth, async (req, res) => {
  ...
  try {
    ...
    for (const assignment of assignments) {
      await ensureDateChecksForAssignment(assignment);
      assignment.recordings = await allQuery(
        'SELECT * FROM recordings WHERE assignment_id = ? ORDER BY created_at DESC',
        [assignment.id]
      );
      assignment.feedback = await allQuery(
        `
      ...
      assignment.day_checks = await allQuery(
        `
...
app.get('/api/assignments/archived', requireAuth, async (req, res) => {
  const { teacherId, studentId } = req.query;
  if (!teacherId) {
...
app.post('/api/assignments', requireAuth, async (req, res) => {
  ...
  if (!normalizedScheduleDates.length) {
    return res.status(400).json({ error: 'Schedule must include at least one date' });
  }
  if (normalizedScheduleDates.length > MAX_SCHEDULE_DAYS) {
    return res.status(400).json({ error: `Schedule cannot exceed ${MAX_SCHEDULE_DAYS} dates` });
  }
  ...
  const scheduleSummary =
    normalizedScheduleDates.length === 1
      ? normalizedStartDate
  ...
  if (![5, 10, 30].includes(normalizedTarget)) {
    return res.status(400).json({ error: 'practiceTarget must be one of: 5, 10, 30' });
  }
  ...
  try {
    const result = await runQuery(
      ...
      [
        ...
        normalizedStartDate,
        normalizedEndDate,
        JSON.stringify(normalizedScheduleDates),
        scheduleSummary,
        normalizedMode,
        normalizedTarget,
        teacherId,
    ...
    const assignment = await getQuery('SELECT * FROM assignments WHERE id = ?', [result.id]);
    await ensureDateChecksForAssignment(assignment);
...
app.patch('/api/assignments/:id/complete', requireAuth, async (req, res) => {
  const assignmentId = req.params.id;
  const { studentId, completed } = req.body;

  try {
    const assignment = await getQuery('SELECT * FROM assignments WHERE id = ?', [assignmentId]);
    if (!assignment) return res.status(404).json({ error: 'Assignment not found' });
    if (Number(assignment.is_archived) === 1) {
      return res.status(400).json({ error: 'Archived assignments cannot be updated' });
    }
    if (Number(assignment.student_id) !== Number(studentId)) {
      return res.status(403).json({ error: 'Only assigned student can update assignment completion' });
    }
    ...
    const updated = await getQuery('SELECT * FROM assignments WHERE id = ?', [assignmentId]);
    if (assignment.status !== 'completed' && nextStatus === 'completed') {
      emitToUser(assignment.teacher_id, 'assignment:completed', {
...
app.patch('/api/assignments/:id/date-check', requireAuth, async (req, res) => {
  const assignmentId = req.params.id;
  const { studentId, taskDate, completed } = req.body;

  if (!studentId || !taskDate) {
    return res.status(400).json({ error: 'studentId and taskDate are required' });
  }
  if (!isIsoDateString(taskDate)) {
    return res.status(400).json({ error: 'taskDate must be valid (YYYY-MM-DD)' });
  }
  ...
  try {
    ...
    if (Number(assignment.student_id) !== Number(studentId)) {
      return res.status(403).json({ error: 'Only assigned student can update daily checks' });
    }
    const previousStatus = assignment.status;

    const scheduleDates = getScheduleDatesForAssignment(assignment);
    if (!scheduleDates.includes(String(taskDate))) {
      return res.status(400).json({ error: 'Selected date is not part of this assignment schedule' });
    }
    ...
    await runQuery(
      `
      UPDATE assignment_date_checks
      SET completed = ?, completed_at = CASE WHEN ? = 1 THEN CURRENT_TIMESTAMP ELSE NULL END
      WHERE assignment_id = ? AND task_date = ?
      `,
      [isCompleted, isCompleted, assignmentId, taskDate]
    );

    const updatedDayCheck = await getQuery(
      `
      SELECT task_date, completed, completed_at
      FROM assignment_date_checks
      WHERE assignment_id = ? AND task_date = ?
    ...
    const completionStats = await getQuery(
      `
      SELECT
        COUNT(*) AS total_dates,
        COALESCE(SUM(completed), 0) AS completed_dates
      FROM assignment_date_checks
      WHERE assignment_id = ?
      `,
      [assignmentId]
    ...
    const updatedAssignment = await getQuery(
      `
    ...
    const payload = {
      ...
      assignmentStatus: updatedAssignment?.status || assignment.status
    };
...
app.delete('/api/assignments/:id', requireAuth, async (req, res) => {
  ...
  try {
    const assignment = await getQuery('SELECT * FROM assignments WHERE id = ?', [assignmentId]);
    if (!assignment) return res.status(404).json({ error: 'Assignment not found' });
    if (Number(assignment.teacher_id) !== Number(teacherId)) {
      return res.status(403).json({ error: 'Only assignment teacher can archive this assignment' });
    }
    if (Number(assignment.is_archived) === 1) {
      return res.status(400).json({ error: 'Assignment is already archived' });
    }
...
app.patch('/api/assignments/:id/restore', requireAuth, async (req, res) => {
  const assignmentId = req.params.id;
  const { teacherId } = req.body || {};
  ...
  try {
    ...
    if (!assignment) return res.status(404).json({ error: 'Assignment not found' });
    if (Number(assignment.teacher_id) !== Number(teacherId)) {
      return res.status(403).json({ error: 'Only assignment teacher can restore this assignment' });
    }
    if (Number(assignment.is_archived) !== 1) {
      return res.status(400).json({ error: 'Assignment is not archived' });
    }
...
app.delete('/api/assignments/:id/permanent', requireAuth, async (req, res) => {
  const assignmentId = req.params.id;
  const { teacherId } = req.body || {};
  ...
  try {
    ...
    if (!assignment) return res.status(404).json({ error: 'Assignment not found' });
    if (Number(assignment.teacher_id) !== Number(teacherId)) {
      return res.status(403).json({ error: 'Only assignment teacher can permanently delete this assignment' });
    }
    if (Number(assignment.is_archived) !== 1) {
      return res.status(400).json({ error: 'Only archived assignments can be permanently deleted' });
    }

    const recordings = await allQuery('SELECT file_path FROM recordings WHERE assignment_id = ?', [assignmentId]);

    await runQuery('BEGIN TRANSACTION');
    try {
      await runQuery('DELETE FROM feedback WHERE assignment_id = ?', [assignmentId]);
      await runQuery('DELETE FROM assignment_date_checks WHERE assignment_id = ?', [assignmentId]);
      await runQuery('DELETE FROM assignment_daily_checks WHERE assignment_id = ?', [assignmentId]);
      await runQuery('DELETE FROM recordings WHERE assignment_id = ?', [assignmentId]);
      await runQuery('DELETE FROM assignments WHERE id = ?', [assignmentId]);
...
app.post('/api/assignments/:id/recordings', requireAuth, upload.single('recording'), async (req, res) => {
  const assignmentId = req.params.id;
  ...
  try {
    ...
    if (!assignment) return res.status(404).json({ error: 'Assignment not found' });
    if (Number(assignment.student_id) !== Number(studentId)) {
      return res.status(403).json({ error: 'Only assigned student can upload recordings' });
    }

    const filePath = `/uploads/${req.file.filename}`;
    const result = await runQuery(
      `
      INSERT INTO recordings (assignment_id, student_id, file_path, original_name)
      VALUES (?, ?, ?, ?)
      `,
      [assignmentId, studentId, filePath, req.file.originalname]
    );

    const recording = await getQuery('SELECT * FROM recordings WHERE id = ?', [result.id]);
    emitToUser(assignment.teacher_id, 'recording:new', {
      assignmentId: Number(assignmentId),
      recording
    });
...
app.post('/api/assignments/:id/feedback', requireAuth, async (req, res) => {
  const assignmentId = req.params.id;
  ...
  try {
    ...
    if (!assignment) return res.status(404).json({ error: 'Assignment not found' });
    if (Number(assignment.teacher_id) !== Number(teacherId)) {
      return res.status(403).json({ error: 'Only assignment teacher can leave feedback' });
    }

    const result = await runQuery(
      `
      INSERT INTO feedback (assignment_id, recording_id, teacher_id, comment)
      VALUES (?, ?, ?, ?)
      `,
      [assignmentId, recordingId || null, teacherId, comment.trim()]
    );
    ...
    emitToUser(assignment.student_id, 'feedback:new', {
      assignmentId: Number(assignmentId),
      feedback
    });
...
app.post('/api/chat', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const savedMessage = await getQuery(
      `
      SELECT m.*, s.name AS sender_name
      FROM chat_messages m
      JOIN users s ON s.id = m.sender_id
